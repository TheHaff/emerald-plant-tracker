...
import { EnvironmentData, Plant, PlantStage } from '../types/api';
...
const STAGE_CONFIG = {
  ...
  drying: { emoji: '☀️', label: 'Drying' },
  curing: { emoji: '🥓', label: 'Curing' },
...
const Dashboard = () => {
  ...
  const fetchLatestEnvironmentDataPerTent = async () => {
    try {
      ...
      const directResponse = await fetch(
        'http://localhost:5000/api/environment/latest-per-tent',
      );
  ...
  const getStageConfig = (stage: PlantStage) => {
                          ...
                          >
                            <span
                              style={{
                                ...
                                border: `1px solid ${
                                  getStageColor(log.stage).border
                                }`,
                                gap: '0.2rem',
                                backdropFilter: 'blur(8px)',
                                WebkitBackdropFilter: 'blur(8px)',
