...
import type { ArchivedGrow } from '../types/api';

const ArchivedTents = () => {
  const [archivedGrows, setArchivedGrows] = useState<ArchivedGrow[]>([]);
  ...
  const handleExportGrow = async (grow: ArchivedGrow) => {
    try {
  ...
  const handleClearTentData = async (tentName: string) => {
  ...
  const handleViewDetails = async (grow: ArchivedGrow) => {
    try {
  ...
  const getArchiveReasonText = (reason: ArchiveReason) => {
    switch (reason) {
      case 'completed':
  ...
  const calculateGrowDuration = (startDate: string, endDate: string) => {
  ...
  return (
    <div className="dashboard-page">
      ...
      {(() => {
        const uniqueTents = [
          ...new Set(archivedGrows.map(grow => grow.grow_tent).filter(Boolean)),
        ];
          ...
          >
            ...
            {uniqueTents.length > 0 ? (
              <>
                ...
                >
                  {uniqueTents.map(tentName => {
                    const tentGrowCount = archivedGrows.filter(
                      grow => grow.grow_tent === tentName,
                    ).length;
                    return (
                      <button
                        key={tentName}
              ...
              <div style={{ textAlign: 'center', padding: '2rem' }}>
                ...
                >
                  Archive plants assigned to grow tents to enable tent-based
                  exports
                </p>
              </div>
            )}
      ...
      >
        Showing {filteredAndSortedGrows.length} of {archivedGrows.length}{' '}
        archived grows
      </div>
...
type ArchivedGrowCardProps = {
  grow: ArchivedGrow;
  onExport: (grow: ArchivedGrow) => void;
  onViewDetails: (grow: ArchivedGrow) => void;
  calculateGrowDuration: (startDate: string, endDate: string) => string;
  getArchiveReasonColor: (reason: string) => string;
  getArchiveReasonText: (reason: string) => string;
};
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <div>
            ...
            >
              {grow.harvest_date
                ? format(new Date(grow.harvest_date), 'MMM dd, yyyy')
                : 'Unknown'}
            </div>
          </div>
        </div>
        ...
        <div style={{ marginBottom: '2rem' }}>
          ...
          >
            ...
            <div
              style={{
                ...
                borderRadius: '20px',
                background: `${getArchiveReasonColor(grow.archive_reason)}20`,
                border: `1px solid ${getArchiveReasonColor(
                  grow.archive_reason,
                )}40`,
                color: getArchiveReasonColor(grow.archive_reason),
                fontSize: '0.875rem',
          ...
          >
            ...
            <div style={{ textAlign: 'center' }}>
              ...
              >
                {grow.harvest_date
                  ? format(new Date(grow.harvest_date), 'MMM dd')
                  : 'Unknown'}
              </div>
            </div>
                ...
                >
                  ...
                  {grow.min_temperature && grow.max_temperature && (
                    <div style={{ fontSize: '0.75rem', color: '#94a3b8' }}>
                      Range: {grow.min_temperature.toFixed(1)}° -{' '}
                      {grow.max_temperature.toFixed(1)}°
                    </div>
                  )}
                </div>
              )}
              ...
              {grow.avg_humidity && (
                ...
                >
                  ...
                  {grow.min_humidity && grow.max_humidity && (
                    <div style={{ fontSize: '0.75rem', color: '#94a3b8' }}>
                      Range: {grow.min_humidity.toFixed(1)}% -{' '}
                      {grow.max_humidity.toFixed(1)}%
                    </div>
                  )}
                </div>
              )}
              ...
              {grow.avg_light_intensity && (
                ...
                >
                  ...
                  {grow.min_light_intensity && grow.max_light_intensity && (
                    <div style={{ fontSize: '0.75rem', color: '#94a3b8' }}>
                      Range: {grow.min_light_intensity.toFixed(0)} -{' '}
                      {grow.max_light_intensity.toFixed(0)} PPFD
                    </div>
                  )}
                </div>
              )}
