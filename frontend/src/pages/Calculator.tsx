...
const nutrientBrands = {
    ...
    products: {
      ...
      supplements: [
        ...
        { name: 'Cannaboost', ratio: 2.0, unit: 'ml/L', optional: true }, // Official CANNA: 20ml/10L
        {
          ...
          optional: true,
          floweringOnly: true,
        }, // Official CANNA: 15ml/10L in flower
      ],
    },
    strengthMultipliers: {
      light: 0.6,
  ...
  'dyna-gro': {
    ...
    products: {
      vegetative: [{ name: 'Foliage Pro 9-3-6', ratio: 1.0, unit: 'tsp/gal' }],
      flowering: [{ name: 'Bloom 3-12-6', ratio: 1.0, unit: 'tsp/gal' }],
      supplements: [
        {
...
type NutrientBrand = keyof typeof nutrientBrands;
type WateringMethod =
  | 'hand-watering'
  | 'drip-system'
  | 'bottom-wicking'
  ...
  | 'deep-water-culture'
  | 'ebb-flow';
type WaterType = 'soft' | 'hard';
type NutrientStrength = 'light' | 'medium' | 'aggressive';
type GrowMedium = 'hydro' | 'soil' | 'perlite';
export type CalculationStage = 'seedling' | 'vegetative' | 'flowering';
type Calculations = {
  tankSize: number;
  brand: string;
...
const NutrientCalculator = () => {
  ...
  const loadPreference = (key: string, defaultValue: number | String) => {
    try {
  ...
  const [selectedBrand, setSelectedBrand] = useState<NutrientBrand>(() =>
    loadPreference('selectedBrand', 'general-hydroponics'),
  );
  const [growthStage, setGrowthStage] = useState<CalculationStage>(() =>
    loadPreference('growthStage', 'vegetative'),
  );
  const [tankSize, setTankSize] = useState<number>(() =>
    loadPreference('tankSize', 50),
  );
  const [waterType, setWaterType] = useState<WaterType>(() =>
    loadPreference('waterType', 'soft'),
  );
  const [growMedium, setGrowMedium] = useState<GrowMedium>(() =>
    loadPreference('growMedium', 'hydro'),
  );
  const [feedingStrength, setFeedingStrength] = useState<NutrientStrength>(() =>
  ...
  const savePreference = (key: string, value: string | number) => {
    try {
  ...
  const handleGrowthStageChange = (newStage: CalculationStage) => {
  ...
  const handleFeedingStrengthChange = (newStrength: NutrientStrength) => {
    ...
    if (newStrength === 'light' && growthStage === 'flowering') {
      toast(
        '💡 Light feeding is typically used for seedlings or young plants',
        {
          icon: '💡',
          duration: 3000,
        },
  ...
  const handleBrandChange = (brand: NutrientBrand) => {
  ...
  const handleGrowMediumChange = (medium: GrowMedium) => {
    setGrowMedium(medium);
    savePreference('growMedium', medium);
  };

  const handleWateringMethodChange = (method: WateringMethod) => {
  ...
  const calculateNutrients = useCallback(() => {
    ...
    calculations.supplements.sort((a, b) => {
      const getMixingPriority = (name: string) => {
    ...
    if (selectedBrand === 'general-hydroponics') {
      calculations.instructions.splice(
        1,
        1,
      ...
      >
        ...
        >
          ...
          >
            ...
            <select
              value={selectedBrand}
              onChange={(e) => handleBrandChange(String(e.target.value))}
              style={{
                width: '100%',
                background: '#2d3748',
                border: '1px solid var(--border)',
            ...
            >
              <option value="general-hydroponics">
                General Hydroponics FloraSeries
              </option>
              <option value="advanced-nutrients">
                Advanced Nutrients pH Perfect GMB
              </option>
          ...
          >
            ...
            >
              <option value="hydro">
                💧 Hydroponic (DWC, Clay Pebbles, Rockwool)
              </option>
              <option value="coco">
                🥥 Coco/Soilless (Coco Coir, Peat Mixes)
              </option>
              <option value="soil">🌱 Soil (Organic, Living Soil)</option>
              <option value="perlite">
                ⚪ Inert Media (Perlite, Vermiculite)
              </option>
            </select>
            <p
              style={{
          ...
          >
            ...
            >
              ...
              <option value="deep-water-culture">
                🌊 Deep Water Culture (100%)
              </option>
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            ...
            <input
              ...
              value={tankSize}
              onChange={(e) =>
                handleTankSizeChange(Number(e.target.value))
              }
              style={{
                width: '80px',
                background: '#2d3748',
        ...
        {calculations && (
          <div style={{ display: 'grid', gap: '1.5rem' }}>
            ...
            >
              ...
              >
                For {calculations.tankSize}L tank at {calculations.strength}{' '}
                strength
                {calculations.targetEC && (
                  <span
                    style={{ color: 'var(--text-muted)', marginLeft: '0.5rem' }}
                  >
                    • Target: {calculations.targetEC} EC /{' '}
                    {calculations.targetTDS} TDS (ppm)
                  </span>
                )}
              </p>
            </div>
            ...
            <div style={{ display: 'grid', gap: '1.5rem' }}>
              ...
              {calculations.supplements.length > 0 && (
                ...
                >
                  ...
                  <div style={{ display: 'grid', gap: '0.75rem' }}>
                    {calculations.supplements.map(
                      (supplement: any, index: any) => (
                        <div
                          key={index}
                          style={{
                            display: 'flex',
              ...
              >
                ...
                >
                  ⚗️ Step {calculations.supplements.length > 0 ? '2' : '1'}:
                  Base Nutrients (Add in this order)
                </h4>
                <div style={{ display: 'grid', gap: '0.75rem' }}>
                  {calculations.baseNutrients.map(
                    (nutrient: any, index: any) => (
                      <div
                        key={index}
                        style={{
                          display: 'flex',
            ...
            >
              ...
              >
                🥼 Step {calculations.supplements.length > 0 ? '3' : '2'}: Final
                Mixing Instructions
              </h4>
              <ol
                style={{
              ...
              >
                <li>
                  <strong>Start with clean water</strong> - Use RO or distilled
                  water if possible
                </li>
                {calculations.supplements.length > 0 && (
                  <li>
                ...
                <li>
                  <strong>Add base nutrients in order</strong> - Follow the
                  numbered steps above
                </li>
                <li>
                  <strong>Mix thoroughly</strong> - Stir for 30 seconds between
                  each addition
                </li>
                <li>
                  <strong>Check pH</strong> - Adjust to 5.5-6.5 for hydro/coco,
                  6.0-7.0 for soil
                </li>
                <li>
                  <strong>Check TDS/PPM</strong> - Should match your feeding
                  strength guidelines
                  {calculations.targetTDS
                    ? ` (target: ${calculations.targetTDS} ppm)`
                    : ''}
                </li>
                <li>
                  <strong>Use fresh</strong> - Mix fresh every 7-10 days for
                  best results
                </li>
              </ol>
            </div>
